...
export default function ReservationSettingsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-surface-900">예약 설정</h1>
      <p className="mt-1 text-sm text-surface-500">서비스/메뉴와 예약 가능 시간을 설정하세요.</p>
      ...
      <div className="mt-6 card-surface p-6">
        ...
        {showServiceForm && (
          <form onSubmit={(e) => { e.preventDefault(); createServiceMutation.mutate({ ...serviceForm, price: parseInt(serviceForm.price), duration: parseInt(serviceForm.duration) }); }} className="mb-4 grid grid-cols-4 gap-3 rounded-lg bg-surface-50 p-4">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => setShowServiceForm(false)} className="rounded-lg border px-3 py-2 text-sm text-surface-600">취소</button>
            </div>
          </form>
        )}
        ...
        <div className="space-y-2">
          {services?.map((s: Record<string, unknown>) => (
            <div key={s.id as string} className="flex items-center justify-between rounded-lg border px-4 py-3">
              <div>
                <p className="font-medium text-surface-900">{s.name as string}</p>
                <p className="text-sm text-surface-500">{String(s.duration)}분 | {formatCurrency(s.price as number)}</p>
              </div>
              <button onClick={() => deleteServiceMutation.mutate(s.id as string)} className="text-surface-400 hover:text-red-500">
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
          ))}
          {!services?.length && <p className="py-4 text-center text-surface-400">등록된 서비스가 없습니다.</p>}
        </div>
      </div>
    </div>
  );
