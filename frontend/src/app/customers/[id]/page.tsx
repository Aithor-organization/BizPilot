...
export default function CustomerDetailPage() {
  ...
  const typeIcons: Record<string, React.ReactNode> = {
    RESERVATION: <CalendarDays className="h-4 w-4 text-brand-500" />,
      ...
      <div className="card-surface p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="mt-2 flex gap-4 text-sm text-surface-500">
          ...
          <div className="text-right">
            <p className="text-sm text-surface-500">누적 결제</p>
            <p className="text-2xl font-bold text-surface-900">{formatCurrency(customer.totalSpent)}</p>
            <p className="text-sm text-surface-500">{customer.totalVisits}회 방문</p>
          </div>
        </div>
      </div>

      <div className="mt-6">
        <h2 className="text-lg font-semibold text-surface-900">활동 이력</h2>
        <div className="mt-4 space-y-3">
          {history?.map((item: Record<string, unknown>, i: number) => (
            <div key={i} className="flex items-start gap-3 rounded-lg border bg-white px-5 py-4">
              <div className="mt-0.5">{typeIcons[item.type as string] || <MessageSquare className="h-4 w-4 text-surface-400" />}</div>
              <div className="flex-1">
                <p className="text-sm font-medium text-surface-900">{item.description as string}</p>
                <p className="text-xs text-surface-500">{formatDateTime(item.date as string)}</p>
              </div>
          ...
          {!history?.length && <p className="py-8 text-center text-surface-400">활동 이력이 없습니다.</p>}
        </div>
      </div>
    </div>
  );
