  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">고객 CRM</h1>
          <p className="mt-1 text-sm text-gray-500">고객 정보를 관리하고 이력을 추적하세요.</p>
        </div>
      ...
      <div className="mt-6">
        <div className="overflow-hidden rounded-xl border bg-white">
          <table className="min-w-full divide-y">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">누적 결제</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">최근 방문</th>
            ...
            <tbody className="divide-y">
              {customers?.data?.map((c: Record<string, any>) => (
                <tr key={c.id as string} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900">{c.totalVisits as number}회</td>
                  <td className="px-6 py-4 text-sm text-gray-900">{formatCurrency(c.totalSpent as number)}</td>
                  <td className="px-6 py-4 text-sm text-gray-500">{c.lastVisitAt ? formatDate(c.lastVisitAt as string) : '-'}</td>
                  <td className="px-6 py-4">
          ...
          {!customers?.data?.length && (
            <div className="py-12 text-center text-gray-400">등록된 고객이 없습니다.</div>
          )}
        </div>
      </div>
    </div>
